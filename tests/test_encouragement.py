...
def test_parse_anki_review_share_command() -> None:
    parsed = parse_journal_message("/ar")
    assert parsed["kind"] == "anki_review_share"
...
def test_telegram_poll_handles_anki_review_share_message() -> None:
    class FakeSender:
        def __init__(self, updates: list[dict]):
            self.updates = updates
            self.sent: list[tuple[str, str]] = []
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        run_with_output(["--db", str(db_path), "user", "set-telegram", "xiaoyu", "1001"])
        fake = FakeSender(
            [{"update_id": 1, "message": {"chat": {"id": 1001, "type": "private"}, "text": "/ar"}}]
        )
        with patch.dict("os.environ", {"LIFE_WEB_BASE_URL": "http://testserver"}, clear=False):
            with patch("life_system.cli.commands._build_telegram_sender_from_env", return_value=fake):
                rc, out = run_with_output(["--db", str(db_path), "telegram", "poll"])
        assert rc == 0
        assert "messages=1" in out
        assert len(fake.sent) == 1
        assert "/share/anki-review?t=" in fake.sent[0][1]


def test_setup_menu_contains_anki_review_share_command() -> None:
    from life_system.infra.telegram_sender import TelegramReminderSender
